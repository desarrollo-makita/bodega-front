...
obtenerInventarioFiltrado(anio: number, mes: number, fechaInventario: string, tipoItem: string, bodega: string) {
  ...
  this.sapService.obtenerInventarioFiltrado(anio, mes, fechaInventario, tipoItem, bodega).subscribe({
    next: (respuesta) => {
...
exportarAExcel(): void {
  ...
  saveAs(blob, `Inventario_${fecha}.xlsx`);
}
